...
//! layer which observes the [`AgentEvent`] stream (specifically
//! [`AgentEvent::NewMessages`]) and persists to a [`ContextStore`].
//!
//! ```text
//! chat() / chat_in_thread()
...
///
/// Yields [`AgentEvent`]s including [`AgentEvent::NewMessages`] whenever
/// new messages are added to the agent state (so outer layers can persist).
...
impl<M: ChatModel> AgentLoop<M> {
    ...
    /// [`AgentEvent::NewMessages`] in the returned stream.
    pub fn run<'a>(
        &'a self,
        ...
        stream! {
            ...
            // Track message count to detect new messages from step()
            let mut known_msg_count = state.messages.len();
            ...
            loop {
                ...
                // ── Emit new messages for outer layer persistence ─────
                if state.messages.len() > known_msg_count {
                    let new_msgs = state.messages[known_msg_count..].to_vec();
                    yield AgentEvent::NewMessages(new_msgs);
                    known_msg_count = state.messages.len();
                }
        ...
        async move {
            match input {
                crate::types::LoopInput::Start {
                    ...
                    Ok(self.run(state, action, true))
                }
                crate::types::LoopInput::Resume { state, results } => {
                    Ok(self.run(state, Action::ToolResults(results), false))
                }
            }
        }
    }
