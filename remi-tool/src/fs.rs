        ...
        async move {
            ...
            Ok(ToolResult::Output(stream! {
                match tokio::fs::write(&path, content.as_bytes()).await {
                    Ok(()) => yield ToolOutput::Result(format!("wrote {} bytes to {}", bytes, path)),
                    Err(e) => yield ToolOutput::Result(format!("error: {}", e)),
                }
            }))
        }
        ...
        -> impl std::future::Future<Output = Result<ToolResult<impl Stream<Item = ToolOutput>>, AgentError>>
    {
        async move {
            ...
            Ok(ToolResult::Output(stream! {
                ...
                match result {
                    Ok(()) => yield ToolOutput::Result(format!("created directory {}", path)),
                    Err(e) => yield ToolOutput::Result(format!("error: {}", e)),
                }
            }))
        }
        ...
        -> impl std::future::Future<Output = Result<ToolResult<impl Stream<Item = ToolOutput>>, AgentError>>
    {
        async move {
            ...
            Ok(ToolResult::Output(stream! {
                ...
                match result {
                    Ok(()) => yield ToolOutput::Result(format!("removed {}", path)),
                    Err(e) => yield ToolOutput::Result(format!("error: {}", e)),
                }
            }))
        }
