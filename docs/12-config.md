...
impl OpenAIClient {
    ...
    pub(crate) fn apply_config_overrides(req: &mut ModelRequest, config: &AgentConfig) {
...
AgentConfig
    ...
    │
    ├──→ Tool 构造期 / ChatCtx metadata   // Tool 不再直接拿到 AgentConfig
    │    └─ tool 自身字段保存静态配置；每次调用的 metadata 通过 ChatCtx 透传
    │
...
当前实现里，`Tool::execute()` 接收的是 `ChatCtx`，而不是直接暴露 `AgentConfig`。因此 tool 获取配置主要有两种方式：

- **构造期注入**：把 API key、客户端、根路径等静态依赖保存在 tool 自身字段里
- **调用期透传**：把本次运行专属信息放进 `LoopInput::Start.metadata`，再从 `ChatCtx::metadata()` 里读取

`AgentConfig` 仍然主要服务于 model 调用配置和 transport headers；它不再作为通用 tool runtime context 对外暴露。
...
impl Tool for WeatherTool {
    ...
    async fn execute(
        ...
        // 从 ChatCtx metadata 读取本次调用的区域配置
        let region = ctx
            .metadata()
        ...
        Ok(ToolResult::Output(stream! {
            ...
            yield ToolOutput::Delta(format!("Fetching weather for {city} in {region}..."));
...
impl Tool for WeatherTool {
    ...
    async fn execute(
        ...
        Ok(ToolResult::Output(stream! {
            ...
            let result = fetch_weather_with_key(api_key, city).await;
            yield ToolOutput::Result(result);
        }))
    }
}
...
- `AgentConfig` 负责 model / transport 层配置
- `ChatCtx` 负责 metadata、user_state、取消信号、调用链上下文
- tool 自身依赖优先在构造期注入，而不是每次从全局配置对象里读取

这让 tool 的依赖边界更清晰，也避免把 model 配置和 tool 运行时状态混成一个上下文对象。

```rust
// 旧设计中的 ToolContext / execute_with_context 已废弃，不再作为当前实现说明。
```
...
impl Agent for MyWasmAgent {
    ...
    fn chat(&self, ctx: ChatCtx, req: ModelRequest)
        -> impl Future<Output = Result<impl Stream<Item = ChatResponseChunk>, AgentError>>
    {
        async move {
            ...
            let api_key = config.api_key
                .ok_or_else(|| AgentError::model("api_key not provided by host"))?;

            // 用配置构造 model client
            let model = OpenAIClient::from_config(&config);

            // 正常使用 model
            let mut stream = model.chat(ctx, ModelRequest {
