...
fn main() {
        ...
        if dist_dir.exists() {
            ...
            panic!(
                "REMI_SKIP_UI_BUILD is set but dist-ui/ does not exist. \
                 Run `npm run build` in remi-agentloop-cli/ui/ first."
            );
        }
    }
        ...
        if dist_dir.exists() {
            ...
            println!(
                "cargo:warning=npm not found; using existing dist-ui/. \
                 Run `npm run build` in remi-agentloop-cli/ui/ to rebuild the web UI."
            );
            return;
        } else {
            panic!(
                "npm not found on PATH and dist-ui/ does not exist. \
                 ...
                 `cd remi-agentloop-cli/ui && npm install && npm run build`"
            );
        }
    }
    ...
    if !install_status.success() {
        panic!("`npm install` failed in remi-agentloop-cli/ui/");
    }
    ...
    if !build_status.success() {
        panic!("`npm run build` failed in remi-agentloop-cli/ui/");
    }
}
